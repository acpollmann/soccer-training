...
By performing our analysis again with the LDA-reduced dataset, we managed a slight improvement to a CV-score of 66.0% and a testing score of 63.8%, with the same activation and number of hidden layers, but with a regularization of ⍺=1.1.
...
Initially, our analysis determined that a linear kernel with C=1.1 was the best, yielding a cross validation score of 60.0% and a testing score of 59.5%. We can then apply LDA to reduce our dataset to two features.

Performing the same analysis as before on both of these reduced datasets, our new best hyperparameters for ternary classification were a radial basis function kernel with C=0.55, which yield a cross validation score of 65.7% and a testing score of 64.7%! This is a 5% improvement, by using 2 features instead of 22! 
...
Now, for binary classification, LDA will always compress the dataset into one feature. Performing a similar cross validation analysis on this single feature dataset, we found that the best hyperparameters for deciding between “Win” and “Not Win” were a linear kernel with C=0.45, which yields a cross validation score of 79.1% and a testing accuracy of 75.0%! It’s incredible that we can achieve this accuracy by looking at a single feature! These binary classification results are on par with our Neural Net’s performance for all 22 features.
